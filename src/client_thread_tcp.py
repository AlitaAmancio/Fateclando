...
def parse_args():
    parser = argparse.ArgumentParser(description="Start the encrypted TCP client.")
    ...
    parser.add_argument("--server", default="127.0.0.1", help="Server IP or host (default: 127.0.0.1).")
...
def start_receive_loop(connection, peer_name, session_key, stop_event):
    system("Receiving thread started.")

    while not stop_event.is_set():
        try:
            encrypted_payload = recv_packet(connection)
...
def main():
    ...
    status("Negotiating secure session key...")
    send_packet(tcp_con, wrap_session_key(peer_public_key, session_key))
    status("Secure session established.")
    system("Type your messages and press Enter. Close the terminal or press Ctrl+C to exit.")

    stop_event = Event()
    t_rec = Thread(target=start_receive_loop, args=(tcp_con, peer_name, session_key, stop_event), daemon=True)
    t_rec.start()
